...
priority_queue<int, vector<int>, greater<int> > min_heep;
...
void switch_threads(int switch_case) {
    ...
    current_running_thread = new_thread_id;
 ...
 */
void signal_handler(int to_do) {

    if (is_block && to_do == BLOCK)
        return;

    if (!is_block && to_do == UNBLOCK)
        return;
 ...
 */
int new_id() {
    int ret_val = min_heep.top(); //Getting the minimum number from the min heep.
    min_heep.pop(); //Popping out the minimum number from the min heep.
    return ret_val;
}
...
int uthread_terminate(int tid) {
    ...
    } else { // ((tid != current_running_thread))If the thread is not the running one
        if (cur_thread->state == READY) { //Removing the thread from the ready queue if needed
            remove_from_queue(tid);
        }

        if (cur_thread->sleeping_timer > 0) {//Removing the thread from the sleeping list if needed
...
int uthread_resume(int tid) {
    ...
    T cur_thread = thread_map[tid];
    if (tid == 0 || cur_thread->state == RUNNING || cur_thread->state == READY) {
        signal_handler(UNBLOCK);
        return SUCCESS;
    }
